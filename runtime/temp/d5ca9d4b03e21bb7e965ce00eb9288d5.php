<?php /*a:2:{s:68:"/var/www/manager_cmlaw/application/home/view/user/system/lingyu.html";i:1546067576;s:61:"/var/www/manager_cmlaw/application/home/view/public/base.html";i:1558926142;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<div class="row">
    <div class="col-md-6">
        <table class="table table-border-order">
            ...
            <tr>
                ...
                <td><?php echo htmlentities($list['lingyu_name']); ?></td>
                <td><?php echo htmlentities($list['lingyu_biaoshi']); ?></td>
                <td class="text-right width160">
                    <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">

                        <button type="button" data-toggle="hr_lingyu" data-align="update" data-bind="<?php echo htmlentities($list['idlingyu']); ?>"
                                class="btn btn-link btn-default">修改
                        </button>
                        <button type="button" data-toggle="hr_lingyu" data-align="delete" data-bind="<?php echo htmlentities($list['idlingyu']); ?>"
    ...
    <div class="col-md-6">
        <form method="post" id='hrCreateForm'>
            ...
            <h4>业务领域信息修改
                <button type="button" data-toggle="hr_lingyu" data-align="create" data-bind=""
                        class="btn btn-link btn-default btn-sm">新增业务领域
                </button>
            </h4>
            <table class="table table-bordered">
                <tbody>
                <tr>
                    <td class="input-title text-right"><span>领域：</span></td>
                    <td>
                        <div class="width-100">
                            <input name="name" type="text" class="form-control" value="<?php echo htmlentities($data['info']['lingyu_name']); ?>">
                        </div>
                    </td>
                </tr>
                <tr>
                    <td class="input-title text-right"><span>标识：</span></td>
                    <td>
                        <div class="width-50">
                            <input name="biaoshi" type="text" class="form-control" value="<?php echo htmlentities($data['info']['lingyu_biaoshi']); ?>">
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <div class="width-100">
                            <textarea name="beizhu" class="form-control" rows="3"><?php echo htmlentities($data['info']['lingyu_beizhu']); ?></textarea>
                        </div>
                    </td>
                </tr>
                </tbody>
            </table>
            <input name="id" type="hidden" value="<?php echo htmlentities($data['info']['idlingyu']); ?>">
            <?php else: ?>
            <h4>业务领域新增</h4>
            <table class="table table-bordered">
                <tbody>
...
<script>
    ...
    $(document).on('click', 'button[type="button"]', function (event) {
        ...
        switch (toggle) {
            case 'hr_lingyu':
                switch (align) {
                    case 'create':
                        $(location).attr('href', web_url + '/home/user/system/type/lingyu');
                        break;
                    case 'update':
                        $(location).attr('href', web_url + '/home/user/systemEdit/type/lingyu/id/' + bind);
                        break;
                    case 'delete':
                        if (confirm('确定删除该业务领域吗？') == true) {
                            $.post(web_url + '/home/user/deleteSystemInfo/type/lingyu', {id: bind}, function (obj, status) {
                                if (status === 'success') {
                                    if (obj.status === '1001') {
    ...
    var fromUrl = web_url + '/home/user/saveLingyuInfo';
