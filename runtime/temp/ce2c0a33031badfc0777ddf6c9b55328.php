<?php /*a:2:{s:61:"/var/www/manager_cmlaw/application/rbac/view/index/index.html";i:1545186979;s:61:"/var/www/manager_cmlaw/application/rbac/view/public/base.html";i:1557302993;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<script src="<?php echo htmlentities(app('config')->get('web_url')); ?>/static/js/laydate.js"></script>
<script src="<?php echo htmlentities(app('config')->get('web_url')); ?>/highcharts/highcharts.js"></script>
<script src="<?php echo htmlentities(app('config')->get('web_url')); ?>/highcharts/modules/exporting.js"></script>

<div class="row">
    <div class="col-md-12">
        <div class="space-border-dashed"></div>
        <div class="row ">
            <div class="col-md-3 col-right-border text-center">
                <h4>参与案件数量</h4>
                <h3 class="text-yeelibaray"><?php echo htmlentities($data['tongji']['anjian']); ?>

                </h3>
            </div>
            <div class="col-md-3 col-right-border text-center">
                <h4>客户数量</h4>
                <h3 class="text-yeelibaray"><?php echo htmlentities($data['tongji']['kehu']); ?>

                </h3>
            </div>
            <div class="col-md-3 col-right-border text-center">
                <h4>今日计时数</h4>
                <h3 class="text-yeelibaray"><?php echo htmlentities($data['tongji']['day']); ?>
                    <small>Unit</small>
                </h3>
            </div>
            <div class="col-md-3 col-right-border text-center">
                <h4>本月计时数</h4>
                <h3 class="text-yeelibaray"><?php echo htmlentities($data['tongji']['yue']); ?>
                    <small>Unit</small>
                </h3>
            </div>
        </div>
    ...
    <div class="col-md-12">
        ...
        <table class="table table-border-order">
            ...
            <tr>
                <td>
                    <div style="width: 100%; height: 300px;" id="workChar"></div>
                </td>
            </tr>
            </tbody>
        </table>
...
<script type="text/javascript">
    $.ajax({
        url: web_url + '/home/index/getWorkChar',
        type: "post",
        dataType: "json",
        success: function (obj) {
            if (obj.status === '1001') {
                // 指定图表的配置项和数据
                var workChar = echarts.init(document.getElementById('workChar'), 'macarons');
                option = {
                    xAxis: {
                        type: 'category',
                        data: obj.data.title
